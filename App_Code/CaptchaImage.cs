...
namespace Link
{
    ...
    public class CaptchaImage
    {
        ...
        public CaptchaImage(string s, Color bc, int width, int height, Color fc)
        {
            this.text = s;
            this.bc = bc;
            this.fc = fc;
        ...
        private void GenerateImage()
        {
            ...
            if (fc.Name.Equals("0"))
            {
                ...
                sBrush = new SolidBrush(Color.FromArgb(red, green, blue));
            }
            else {// Use this if The user Defines Custom Background Color
                sBrush = new SolidBrush(fc);
            }
            
            g.FillPath(sBrush, path);

            // Iterate over every pixel
            double distort = RNG.Next(5, 10) * (RNG.Next(5) == 1 ? 1 : -1);
